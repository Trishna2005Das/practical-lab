...
private:
    ...
    void calculateTime(clock_t start, clock_t end, int i) {
        timediff[i] = double(end - start) / CLOCKS_PER_SEC;
    }

    void displayTimeDiff() {
        cout << "Time taken for each operation:" << endl;
        cout << "Build max heap: " << timediff[0] << " seconds" << endl;
        cout << "Insert key: " << timediff[1] << " seconds" << endl;
        cout << "Extract key: " << timediff[2] << " seconds" << endl;
    }
